...
How do we know the inverse of $\mathbf{X}^\top \mathbf{X}$ exists?
Luckily, there's a simple condition: $\mathbf{X}^\top \mathbf{X}$ is invertible if and only if the columns of $\mathbf{X}$ are linearly independent.
...
These properties are nice, but frankly they're kind of dime-a-dozen.  Lots of estimators are unbiased, consistent, and asymptotically normal.
(How many do I mean by "lots"?  Literally infinitely many.)
...
The condition we need is that we have *spherical errors*:
$$
...
If the spherical errors assumption holds, then OLS is the most efficient unbiased estimator of the linear model coefficients $\beta$.
In other words, if we have another estimator of $\beta$ that has smaller standard errors, then that estimator must be biased.
This conclusion is known as the Gauss-Markov Theorem.[^gauss-markov]

[^gauss-markov]: Until recently, the Gauss-Markov Theorem was interpreted as OLS being the "best linear unbiased estimator"---or, cutesily, BLUE---under spherical errors.  This left open the possibility of an unbiased estimator of the linear model coefficients that has standard errors smaller than OLS, but is a nonlinear function of $\mathbf{Y}$.  @portnoy2022linearity has shown that all unbiased estimators of the linear model coefficients are linear estimators, which means it's redundant to call OLS the best *linear* unbiased estimator.  It's just the best unbiased estimator.

Much later in the course, we will encounter ridge regression, an estimator that has lower standard errors than OLS.  The Gauss-Markov theorem tells us that we're making a tradeoff when we use ridge regression---that we're taking on some bias in exchange for the reduction in variance.
