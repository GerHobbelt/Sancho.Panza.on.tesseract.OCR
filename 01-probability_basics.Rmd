...
```{r nice-fig-101A, fig.cap='Three Card Game', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/three_spot_card_game.png")
```

What do you think the probability is that you will win? A **fair** game is one in which the payoffs and probabilities give neither you nor your opponent (often called the *house*) a systematic advantage. Does this game sound like a “fair” game to you? Explain.
...
We often use function notation when describing probabilities. For a well-defined event A in a probability experiment we will use **P(A)** to represent the probability event A occurs. We also informally use short descriptions of events combined with probability function notation as long as the context is clear. For example, when tossing a die, if it is clear from the context that we mean the event of getting a six, we might write $P(Six) = 1/6$. 
...
```{r nice-fig-101B, fig.cap='Sample Space for One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_sample_space.png")
```
...
```{r nice-fig-101C, fig.cap='An Even Outcome on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_even.png")
```
...
```{r nice-fig-101D, fig.cap='An Outcome of Six on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_six.png")
```
...
```{r nice-fig-101E, fig.cap='An Outcome Less Than Three on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_less_than_3.png")
```


## The Complement: NOT {#the_complement}

We now consider compound events consisting of certain logical combinations of other events. We begin with the complement. The complement of an event A consists of all the outcomes that are NOT outcomes of A. We use the notation **not A** to represent the complement of A. (Note, some sources use $\bar{A}$ though this can be confusing as the same symbol in other contexts represents the mean of a sample. Other sources may use yet different notation for the complement including $E^c$ and $\tilde E$.)
...
```{r nice-fig-101F, fig.cap='Being Both Even and a Six on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_even_six.png")
```

The only member of the sample space that is both even and less than three is two so $P(E \ and \ L)=1/6$.

```{r nice-fig-101G, fig.cap='Being Both Even and Less Than Three on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_even_less_than_3.png")
```
...
For the experiment of rolling one die and the events E, S, and L defined above, we see events S and L are mutually exclusive since one cannot roll a six and a number less than three at the same time on the same die. This is indicated in the diagram below showing there is no overlap.

```{r nice-fig-101H, fig.cap='Being Both a Six and Less Than Three on One Die Cannot Occur', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_six_less_than_3.png")
```
...
```{r nice-fig-104, fig.cap='Three Outcomes are Even or a Six', out.width='30%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_even_six.png")
```
...
```{r nice-fig-105, fig.cap='Four Outcomes are Even or Less Than Three', out.width='30%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_even_less_than_3.png")
```
...
```{r nice-fig-106, fig.cap='Three Outcomes are Less Than Three or a Six', out.width='30%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_six_less_than_3.png")
```
...
We consider one more way of “connecting” simple events to form a compound event. The conditional event **$X|Y$** means the event X given we know event Y occurred and we pronounce it "X given Y". As an example, if we were considering the sample space of students in class and we were interested in the event $Female \ | \ Sophomore$ we would mean given Sophomores only, what is the probability of being Female. Note, this is different from $Sophomore \ | \ Female$ which is the probability you are Sophomore given you are Female. 

Tossing one die with the events as described above, $P(L \ | \ E)$ represents the probability of L given we know E occurred, that is, the probability of a number less than three given we know the number is even. Knowing it is even narrows down the sample space to three options, $\{2,4,6\}$.

```{r nice-fig-101J, fig.cap='Revised Sample Space Given it is Even on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_given_even.png")
```

Of those three options, only one of those is less than three, thus, $P(L \ | \ E) = 1/3$. 

```{r nice-fig-101K, fig.cap='Being Less Than Three Given it is Even on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_less_than_3_given_even.png")
```

Conditional probabilities are very important and a little more practice is in order. First, we note that the event $L \ | E$ means something different than the event $E \ | L$. To find $P(E \ | L)$ we note that given event L occurred means the new sample space has just two elements, $\{1,2\}$.

```{r nice-fig-101L, fig.cap='Revised Sample Space Given it is Less Than Three on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_given_less_than_3.png")
```

And of these two options only one is even. Thus, $P(E \ | L)=1/2$. 

```{r nice-fig-101M, fig.cap='Being Even Given it is Less Than Three on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_even_given_less_than_3.png")
```
...
```{r nice-fig-101N, fig.cap='Being Six Given it is Even on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_six_given_even.png")
```

The event $E \ | S$ has a different meaning. Given that event S occurred we know the outcome was a six so the new sample space has only this one outcome, $\{6\}$, and it is even. Thus, $P(E \ | S)=1/1=1$.

```{r nice-fig-101P, fig.cap='Being Even Given it is a Six on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_even_given_six.png")
```
...
```{r nice-fig-101Q, fig.cap='Being Six Given Less Than Three on One Die', out.width='60%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/one_die_six_given_less_than_3.png")
```
