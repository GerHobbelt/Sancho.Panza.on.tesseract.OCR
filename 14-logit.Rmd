...
In practice, such predictions usually occur for observations far away from the central tendency of the data.[^mean-pass]

[^mean-pass]: Remember the following property of OLS: if we set every covariate to its sample mean, then the OLS prediction equals the sample mean of the dependent variable.  So if the dependent variable is binary, the linear model predictions will be valid probabilities as long as all the covariates are close to their sample means.
...
[^nls]: You *can* estimate these models with least squares, such as via the nonlinear least squares function `nls()` in R.  However, nonlinear least squares is inefficient relative to maximum likelihood estimation, and is usually no simpler to undertake computationally.
...
$$
\hat{p}_{\text{MLE}}(\mathbf{Z}) = \frac{1}{N} \sum_{n=1}^N Z_n.
$$
...
dat_lik_unif <- tibble(
  theta = seq(0, 15, length.out = 501),
  lik = if_else(theta < 9, 0, 1 / theta^3)
)
ggplot(dat_lik_unif, aes(x = theta, y = lik)) +
  geom_line(data = dat_lik_unif |> filter(theta < 9)) +
  geom_line(data = dat_lik_unif |> filter(theta >= 9)) +
  annotate("point", x = 9, y = 0, shape = 21, fill = "white") +
  annotate("point", x = 9, y = 1 / 9^3) +
...
1.  Start with an initial guess for $\hat{\beta}$.
    For example, you might use the OLS estimates for the linear probability model.
...
<!-- https://cran.r-project.org/web/packages/margins/index.html -- margins package has been dropped -->


```{r margins-pkg, eval=FALSE}
library("margins")
