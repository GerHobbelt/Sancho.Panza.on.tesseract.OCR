...
What if you get one roll of a fair die and if a six comes up you win $\$100$. What is the true value of this proposition, that is, what would be a fair price for this risk/reward opportunity? Equivalently, we can ask how much we would win on average. We call this theoretical average the mathematical expectation or expected value. Since the probability of rolling a six is $\frac{1}{6}$, we would win $\$100$ about once every six tries so on average we would win $\frac{1}{6}$ of $\$100$ so the expected value is $100 \times\frac{1}{6}=\$16.67$.

Suppose we also would win $\$10$ if we roll a four or a five. This would sweeten the pot. And if the pot is sweetened that means the expected value increases. But, by how much? For this portion of the bet, we would win $\$10$ $\frac{1}{3}$ of the time for an additional $10 \times\frac{1}{3}=\$3.33$ on average. 

We can put these two together to get the total expected value: 

$$100 \times \frac{1}{6} + 10 \times \frac{1}{3} = \$16.67 + \$3.33 = \$20$$
...
Outcome | X | P(X) | X*P(X)
-------------:|:-------------:|:------------:|:------------
Roll 6 | +100 | $\frac{1}{6}$ | 16.67
Roll 4 or 5 | +10 | $\frac{1}{3}$ | 3.33
Roll 1, 2, or 3 | 0 | $\frac{1}{2}$ | 0


The expected value for the game Win Six is the sum which equals $\$20$. 
...
Outcome | X | P(X) | X*P(X)
-------------:|:-------------:|:------------:|:------------
Number occurs 0 times | -1 | $\frac{125}{216}$ | $-1 \cdot \frac{125}{216}$
Number occurs 1 time | +1 | $\frac{75}{216}$ | $+1 \cdot \frac{75}{216}$
Number occurs 2 times | +2 | $\frac{15}{216}$ | $+2 \cdot \frac{15}{216}$
Number occurs 3 times | +3 | $\frac{1}{216}$ | $+3 \cdot \frac{1}{216}$


Calculating the mean/expected value by finding the sum shows 
...
We played Deal or No Deal and were given the following offers. 

(a) In one round we were given the offer of $\$289,154$ with six suitcases left containing $\$1,\$400,\$5000,\$500000,\$750000,$ and $\$1000000$. 

<!-- https://stackoverflow.com/questions/72434109/including-symbol-in-table-caption-of-bookdown-latex-table -->

(ref:nice-fig-810-cap) Banker offers $289,154.

```{r nice-fig-810, fig.cap='(ref:nice-fig-810-cap)', out.width='70%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/deal_no_deal_round4.png")
```

Then, after opening two more suitcases leaving $\$1,\$400,\$500000,$ and $\$1000000$ we were offered $\$317,393$. 

(ref:nice-fig-811-cap) Banker offer increases to $317,393.

```{r nice-fig-811, fig.cap='(ref:nice-fig-811-cap)', out.width='70%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/deal_no_deal_round5.png")
```

Use mathematical expectation to explain why it makes sense that the banker's offer increased.


(b) In the next round we opened one more suitcase and the $\$500000$ suitcase was now gone and only the $\$1, \$400$, and $\$1000000$ were left. 

(ref:nice-fig-812-cap) Banker offers now $294,991.

```{r nice-fig-812, fig.cap='(ref:nice-fig-812-cap)', out.width='70%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("01-basics-figures/deal_no_deal_round6.png")
```


The banker's offer was now $\$294991$. Why did the banker's offer decrease? Explain.

(c) From there, things got really bad for us. We opened one suitcase and lost the $\$1000000$ case. The banker offered us $\$186$. Who cares! Still, describe in general how the tool of mathematical expectation can assist in decision making in the game of Deal or No Deal.
