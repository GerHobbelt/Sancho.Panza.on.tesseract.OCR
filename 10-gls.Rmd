...
1.  That $\hat{\beta}_{\text{OLS}}$ is efficient---no other unbiased estimator has smaller standard errors.
...
Autocorrelation usually arises in clustered data (which we will cover very soon) or in time series analysis (which is beyond the scope of this course).
For now, we'll focus on solving the problems that arise from heteroskedasticity.
...
$$
If you plug $\Omega = \mathbf{I}_N$ into this formula, you'll end up with the OLS estimator.
Hence the name *generalized* least squares: OLS is a special case of GLS.

Similar to the formula for OLS, the variance of GLS is
$$
V[\hat{\beta}_{\text{GLS}} \,|\, \mathbf{X}] = \sigma^2 (\mathbf{X}^\top \Omega^{-1} \mathbf{X})^{-1}.
$$

Like OLS, GLS is unbiased and consistent as long as we have strict exogeneity.  Even if we get $\Omega$ wrong---i.e., we misspecify the structure of the error variance---GLS will still "work" in the sense of giving us an unbiased and consistent estimate of the coefficients.
To confirm this, we can once again take the expected value of the estimator:
$$
\begin{aligned}
...
2.  GLS is efficient, per the Gauss-Markov theorem: there is no other unbiased estimator with lower standard errors.
...
\mathbf{X}^* = \begin{bmatrix}
  x_{11} / \sqrt{\omega_{1}} & x_{12} / \sqrt{\omega_{1}} & \cdots & x_{1K} / \sqrt{\omega_{1}} \\
  x_{21} / \sqrt{\omega_{2}} & x_{22} / \sqrt{\omega_{2}} & \cdots & x_{2K} / \sqrt{\omega_{2}} \\
  \vdots & \vdots & \ddots & \vdots \\
  x_{N1} / \sqrt{\omega_{N}} & x_{N2} / \sqrt{\omega_{N}} & \cdots & x_{NK} / \sqrt{\omega_{N}}
\end{bmatrix},
...
$$
In this scheme, the original White estimator is called the "HC0" estimator.

HC1 is the default estimator used by Stata when you run a regression with the `, robust` option.
If for some reason you ever need an exact replication of a regression someone did with "robust standard errors", you'll most likely want to use the HC1 option.
However, HC1 is not actually the best correction for small samples.
@long2000using run simulations and find that the HC3 estimate provides a better estimate in small samples.[^colada]
HC3 works with a slightly different $\hat{\mathbf{U}}$ matrix than HC0 or HC1: a diagonal matrix where each $n$'th diagonal entry is
$$
\frac{\hat{e}_n^2}{(1 - \mathbf{x}_n^\top (\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{x}_n)^2}.
$$
Once again, I am not going to try to explain why this works.
Suffice to say, econometricians much smarter than me think it is the best small-sample correction, and luckily it is the default in the R package we will use.

[^colada]: [This blog post by Uri Simonsohn](http://datacolada.org/99) drew my attention to the use of HC1 as the Stata default and the superiority of HC3 in small samples.
...
The default here is the HC3 estimate.

```{r hccm}
vcv_hc3 <- vcovHC(ols_salaries)
vcv_hc3
```
...
```{r hccm-wls}
vcv_hc3_wls <- vcovHC(wls_salaries)
coeftest(wls_salaries)  # Original
coeftest(wls_salaries, vcov = vcv_hc3_wls)  # Corrected
```
